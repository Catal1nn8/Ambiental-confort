...
float tp(float a=17.27, float b=237.7) 
{
  static float F;
  static float TP;
  int32_t temp, hum, press, gas;
  BME680.getSensorData(temp,hum,press,gas); // Get the most recent values from the device
  float temperatura = temp/100;
  float umiditate = hum/1000;
  F = a*temperatura/(b+temperatura)+log(umiditate/100);
  TP = b*F/(a-F);
  return(TP);
...
//  > 21.0 rosu
  ...
  if ( _tp <= 9 ) {
    ...
    digitalWrite(11, LOW);   //green
      ...
      DebugPort.println("Disconfort");
  } else if ( _tp > 21 ) {
    digitalWrite(13, HIGH);   //red
    digitalWrite(12, LOW);   //yellow
    digitalWrite(11, LOW);   //green
      DebugPort.println("Periculos");
  } else {
    DebugPort.println("Inafara intervalului");
  }
...
//relay
  BME680.getSensorData(temperature,humidity,pressure,gas); // Get most recent readings
int SENSIBILITATE = 31; // AICI SETEZ CAT SA FIE DE SENSIBIL LA SCHIMBAREA VALORILOR umiditatii

       if (temperature/100.0 < SENSIBILITATE)
          {
            Serial.print(" <<< ");
